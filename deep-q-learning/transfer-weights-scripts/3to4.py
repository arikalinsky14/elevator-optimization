...
def transfer_weights(old_model, new_model):
    """
    Transfers weights from the old model (input size 31) to the new model (input size 32).
    For the first Dense layer, the mapping is:
      - new_kernel[0:22, :] = old_kernel[0:22, :]
      - new_kernel[22:24, :] = old_kernel[22:24, :] (first two capacity neurons)
      - new_kernel[24, :] = 0 (newly added neuron)
      - new_kernel[25:32, :] = old_kernel[24:31, :] (remaining neurons)
    ...
    old_kernel, old_bias = old_weights  # old_kernel shape should be (31, 64)
    ...
    new_kernel, new_bias = new_weights  # new_kernel shape should be (32, 64)
    
    # Verify expected shapes
    if old_kernel.shape != (31, 64) or new_kernel.shape != (32, 64):
    ...
    new_kernel[0:22, :] = old_kernel[0:22, :]
    # Copy first three capacity neurons
    new_kernel[22:24, :] = old_kernel[22:24, :]
    # Set row 24 (new capacity neuron) to small random values
    new_kernel[24:25, :] = tf.keras.initializers.HeNormal()(shape=(1, 64)) * 0.01  # Tiny scale
    # Copy the remaining rows
    new_kernel[25:32, :] = old_kernel[24:31, :]
...
def compare_network_weights(old_model, new_model, print_elements=5):
    ...
    Special handling for the first Dense layer's kernel:
      ...
      - New kernel rows 22-23 should match old kernel rows 22-23 (first three capacity neurons)
      - New kernel row 24 should be near zero (new capacity neuron)
      - New kernel rows 25-31 should match old kernel rows 24-30
    """
        ...
        for weight_idx, (old_w, new_w) in enumerate(zip(old_weights, new_weights)):
            ...
            if old_w.shape == new_w.shape:
                ...
                if layer_idx == 0 and weight_idx == 0:
                    old_kernel = old_w   # Expected shape: (31, 64)
                    new_kernel = new_w   # Expected shape: (32, 64)
                    ...
                    # First three capacity neurons (rows 22-23)
                    print("  --- Existing Capacity Neurons (rows 22-23) ---")
                    print(f"  Old rows 22-23 (first {print_elements} elems):")
                    print(f"    {old_kernel[22][:print_elements]}")
                    print(f"    {old_kernel[23][:print_elements]}")
                    print(f"  New rows 22-23 (first {print_elements} elems):")
                    print(f"    {new_kernel[22][:print_elements]}")
                    print(f"    {new_kernel[23][:print_elements]}")
                    
                    # New capacity neuron (row 24)
                    print("  --- New Capacity Neuron (row 24) ---")
                    print(f"  Mean value: {np.mean(new_kernel[24]):.4f}, Std: {np.std(new_kernel[24]):.4f}")
                    print(f"  New row 24 (first {print_elements} elems): {new_kernel[24][:print_elements]}")
                    
                    # Bottom section: new rows 25-31 should equal old rows 24-30
                    bottom_old = old_kernel[24:31, :]
                    bottom_new = new_kernel[25:32, :]
                    diff_bottom = bottom_new - bottom_old
                    print("  --- Bottom Section (rows 25-31) ---")
                    print(f"  Mean diff: {np.mean(diff_bottom):.4f}, Std diff: {np.std(diff_bottom):.4f}")
                    print(f"  Old row 24 (first {print_elements} elems): {bottom_old[0][:print_elements]}")
                    print(f"  New row 25 (first {print_elements} elems): {bottom_new[0][:print_elements]}")
...
def main():
    # Define parameters
    action_size = 3
    old_state_size = 31  # 3-capacity network input dimension
    new_state_size = 32  # 4-capacity network input dimension
    
    old_model_path = r"C:\Users\arika\OneDrive\Desktop\Python Code\savedModels\threeCapacity"
    new_model_path = r"C:\Users\arika\OneDrive\Desktop\Python Code\savedModels\fourCapacity"
    
    print("Loading 3-capacity model from:", old_model_path)
    old_model = tf.keras.models.load_model(old_model_path)
    
    print("Building 4-capacity model with input dimension:", new_state_size)
    ...
    print("Saving 4-capacity model to:", new_model_path)

...
def transfer_weights(old_model, new_model):
    """
    Transfers weights from the old model (input size 33) to the new model (input size 34).
    For the first Dense layer, the mapping is:
      - new_kernel[0:22, :] = old_kernel[0:22, :]
      - new_kernel[22:26, :] = old_kernel[22:26, :] (first five capacity neurons)
      - new_kernel[26, :] = 0 (newly added neuron)
      - new_kernel[27:34, :] = old_kernel[26:33, :] (remaining neurons)
    ...
    old_kernel, old_bias = old_weights  # old_kernel shape should be (33, 64)
    ...
    new_kernel, new_bias = new_weights  # new_kernel shape should be (34, 64)
    
    # Verify expected shapes
    if old_kernel.shape != (33, 64) or new_kernel.shape != (34, 64):
    ...
    new_kernel[0:22, :] = old_kernel[0:22, :]
    # Copy first five capacity neurons
    new_kernel[22:26, :] = old_kernel[22:26, :]
    # Set row 26 (new capacity neuron) to small random values
    new_kernel[26:27, :] = tf.keras.initializers.HeNormal()(shape=(1, 64)) * 0.01  # Tiny scale
    # Copy the remaining rows
    new_kernel[27:34, :] = old_kernel[26:33, :]
...
def compare_network_weights(old_model, new_model, print_elements=5):
    ...
    Special handling for the first Dense layer's kernel:
      ...
      - New kernel rows 22-25 should match old kernel rows 22-25 (first five capacity neurons)
      - New kernel row 26 should be near zero (new capacity neuron)
      - New kernel rows 27-33 should match old kernel rows 26-32
    """
        ...
        for weight_idx, (old_w, new_w) in enumerate(zip(old_weights, new_weights)):
            ...
            if old_w.shape == new_w.shape:
                ...
                if layer_idx == 0 and weight_idx == 0:
                    old_kernel = old_w   # Expected shape: (33, 64)
                    new_kernel = new_w   # Expected shape: (34, 64)
                    ...
                    # First five capacity neurons (rows 22-25)
                    print("  --- Existing Capacity Neurons (rows 22-25) ---")
                    print(f"  Old rows 22-25 (first {print_elements} elems):")
                    print(f"    {old_kernel[22][:print_elements]}")
                    print(f"    {old_kernel[23][:print_elements]}")
                    print(f"    {old_kernel[24][:print_elements]}")
                    print(f"    {old_kernel[25][:print_elements]}")
                    print(f"  New rows 22-25 (first {print_elements} elems):")
                    print(f"    {new_kernel[22][:print_elements]}")
                    print(f"    {new_kernel[23][:print_elements]}")
                    print(f"    {new_kernel[24][:print_elements]}")
                    print(f"    {new_kernel[25][:print_elements]}")
                    
                    # New capacity neuron (row 26)
                    print("  --- New Capacity Neuron (row 26) ---")
                    print(f"  Mean value: {np.mean(new_kernel[26]):.4f}, Std: {np.std(new_kernel[26]):.4f}")
                    print(f"  New row 26 (first {print_elements} elems): {new_kernel[26][:print_elements]}")
                    
                    # Bottom section: new rows 27-33 should equal old rows 26-32
                    bottom_old = old_kernel[26:33, :]
                    bottom_new = new_kernel[27:34, :]
                    diff_bottom = bottom_new - bottom_old
                    print("  --- Bottom Section (rows 27-33) ---")
                    print(f"  Mean diff: {np.mean(diff_bottom):.4f}, Std diff: {np.std(diff_bottom):.4f}")
                    print(f"  Old row 26 (first {print_elements} elems): {bottom_old[0][:print_elements]}")
                    print(f"  New row 27 (first {print_elements} elems): {bottom_new[0][:print_elements]}")
...
def main():
    # Define parameters
    action_size = 3
    old_state_size = 33  # 5-capacity network input dimension
    new_state_size = 34  # 6-capacity network input dimension
    
    old_model_path = r"C:\Users\arika\OneDrive\Desktop\Python Code\savedModels\fiveCapacity"
    new_model_path = r"C:\Users\arika\OneDrive\Desktop\Python Code\savedModels\sixCapacity"
    
    print("Loading 5-capacity model from:", old_model_path)
    old_model = tf.keras.models.load_model(old_model_path)
    
    print("Building 6-capacity model with input dimension:", new_state_size)
    ...
    print("Saving 6-capacity model to:", new_model_path)

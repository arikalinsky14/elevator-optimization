...
def transfer_weights(old_model, new_model):
    """
    Transfers weights from the old model (input size 29) to the new model (input size 30).
    For the first Dense layer, the mapping is:
      - new_kernel[0:22, :] = old_kernel[0:22, :]
      - new_kernel[22, :] = 0 (newly added neuron)
      - new_kernel[23:30, :] = old_kernel[22:29, :]
    ...
    new_kernel, new_bias = new_weights  # new_kernel shape should be (30, 64)
    
    # Verify expected shapes
    if old_kernel.shape != (29, 64) or new_kernel.shape != (30, 64):
        raise ValueError("Unexpected kernel shapes: old_kernel {}, new_kernel {}".format(old_kernel.shape, new_kernel.shape))
    
    # Transfer weights:
    # Copy first 22 rows
    new_kernel[0:22, :] = old_kernel[0:22, :]
    # Set row 22 (1 neuron) to small random values
    new_kernel[22:23, :] = tf.keras.initializers.HeNormal()(shape=(1, 64)) * 0.01  # Tiny scale
    # Copy the remaining 7 rows from the old kernel into rows 23-29
    new_kernel[23:30, :] = old_kernel[22:29, :]
...
def compare_network_weights(old_model, new_model, print_elements=5):
    ...
    Special handling for the first Dense layer's kernel:
      ...
      - New kernel row 22 should be near zero (new neuron)
      - New kernel rows 23-29 should match old kernel rows 22-28
    """
        ...
        for weight_idx, (old_w, new_w) in enumerate(zip(old_weights, new_weights)):
            ...
            if old_w.shape == new_w.shape:
                ...
                if layer_idx == 0 and weight_idx == 0:
                    ...
                    new_kernel = new_w   # Expected shape: (30, 64)
                    ...
                    # Middle section: row 22 in new (should be near zero)
                    middle_new = new_kernel[22:23, :]
                    print("  --- Middle Section (row 22, new neuron) ---")
                    print(f"  Mean value: {np.mean(middle_new):.4f}, Std: {np.std(middle_new):.4f}")
                    print(f"  New row 22 (first {print_elements} elems): {middle_new[0][:print_elements]}")
                    
                    # Bottom section: new rows 23-29 should equal old rows 22-28
                    bottom_old = old_kernel[22:29, :]
                    bottom_new = new_kernel[23:30, :]
                    diff_bottom = bottom_new - bottom_old
                    print("  --- Bottom Section (rows 23-29) ---")
                    ...
                    print(f"  New row 23 (first {print_elements} elems): {bottom_new[0][:print_elements]}")
...
def main():
    # Define parameters
    action_size = 3
    old_state_size = 29  # original network input dimension
    new_state_size = 30  # new network input dimension with one additional neuron
    
    old_model_path = r"C:\Users\arika\OneDrive\Desktop\Python Code\savedModels\SixRequests"
    new_model_path = r"C:\Users\arika\OneDrive\Desktop\Python Code\savedModels\twoCapacity"
    
    print("Loading original model from:", old_model_path)
    old_model = tf.keras.models.load_model(old_model_path)
    
    print("Building new model with input dimension:", new_state_size)
